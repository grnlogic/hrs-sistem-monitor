...
export default function EmployeeDetailPage() {
  ...
  const totalPiutangAktif = useMemo(() => {
    return loans
      .filter((l) => l.aktif)
      .reduce((sum, l) => sum + Number(l.sisaSaldo || 0), 0);
  }, [loans]);
  ...
  const tabGridClass = showAttendanceTab ? "grid w-full grid-cols-8" : "grid w-full grid-cols-7";
    ...
    <div className="space-y-6">
      ...
      >
        <TabsList className={tabGridClass}>
          <TabsTrigger key="tab-personal" value="personal">Data Pribadi</TabsTrigger>
          <TabsTrigger key="tab-salary" value="salary">Riwayat Gaji</TabsTrigger>
          {showAttendanceTab && <TabsTrigger key="tab-attendance" value="attendance">Riwayat Absensi</TabsTrigger>}
          <TabsTrigger key="tab-izin-sakit" value="izin-sakit">Izin &amp; Sakit</TabsTrigger>
          <TabsTrigger key="tab-leave" value="leave">Riwayat Cuti</TabsTrigger>
          <TabsTrigger key="tab-violations" value="violations">Pelanggaran</TabsTrigger>
          <TabsTrigger key="tab-files" value="files">
            Dokumen ({uploadedFiles.length})
          </TabsTrigger>
          <TabsTrigger key="tab-loans" value="loans">Piutang &amp; Cicilan</TabsTrigger>
        </TabsList>

        <TabsContent value="personal" className="space-y-4">
          <PersonalInfoTab employee={employee} leaveInfo={leaveInfo} totalPiutangAktif={totalPiutangAktif} />
        </TabsContent>
